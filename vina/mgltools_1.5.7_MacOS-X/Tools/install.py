...
caddshPath = path.join(bindir, 'raccoon')
f = open(caddshPath, 'w')
f.writelines(tplLines)
f.write(caddlines)
f.close()
...
# Make pythonsh script
if sys.platform == 'darwin':
    #comment open -a XQuartz
    ...
    searchLine = "open -a XQuartz"
    lineInd = [nn for nn, x in enumerate(tplLines) if x.find(searchLine) != -1]
    if len(lineInd):
        for nn in lineInd:
            tplLines[nn] = "#"+tplLines[nn]
...
if path.exists(path.join(bindir, "babel")):
